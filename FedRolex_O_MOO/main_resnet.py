...
parser.add_argument('--moo_method', default=None, type=str)
...
cfg['moo_method'] = args['moo_method']
...
def test(dataset, data_split, label_split, model, logger, epoch, local, user_idx):
    with torch.no_grad():
        model.train(False)
        ...
        for k in range(len(local)):
            processes = []
            processes.append(local[k % len(local)].test_model_for_user.remote(user_idx[k], [dataset_id, data_split_id, model_id, label_split_id], epoch))
